...
public class CarEngine : MonoBehaviour
{
    ...
    public const float MOTOR_BRAKE = 50f;
    ...
    void FixedUpdate()
    {
        ...
        if (car.getEngineEnable())
        {
            foreach (Wheel wheel in wheels.throttleWheels)
            {
                ...
                Debug.Log("Engine, FixedUpdate : realTorque :" + currMotorTorque);
    ...
    private void SetTorque()
    {
        ...
        {
            var fakeSpeed = (0.5f * (speed + 1f));
            currMotorTorque = (maxSpeed / Mathf.Clamp(fakeSpeed, 90f, 1000f * maxSpeed)) * MOTOR_TORQUE;
            //currMotorTorque = MOTOR_TORQUE;
        }
    ...
    {
        MovementInputs.y = input;
    }
    public void SetInputX(float input)
    {
        MovementInputs.x = input;
    }
}
